/* ==========================================================================
   Radim Theiner — script.js (clean master)
   - Theme toggle + contact overlay
   - Smooth scrolling + section arrows + hash routing
   - Biker animation (RAF, stable)
   - Dark-mode cursor
   - Typewriter
...
(() => {
  ...
  */
  ...
  function initSmoothScroll() {
    ...
    function scrollToElementTopWithGap(el) {
      if (!el) {
        return;
      }
    ...
    function scrollToNextSection(fromEl) {
      ...
      scrollToElementTopWithGap(next);
    }
    ...
    document.querySelectorAll("[data-scroll]").forEach((el) => {
      el.addEventListener("click", (e) => {
        ...
        if (mode === "next") {
          // Keep your original behavior: disable "next" arrows on <=1024px
    ...
    // Intercept ALL hash links and smooth scroll
    ...
    window.__scrollToElementTopWithGap = scrollToElementTopWithGap;
  }
  ...
  function initBiker() {
    ...
    function onScroll() {
      ...
      if (nameWrapper) {
        nameWrapper.classList.toggle("name-scrolled", lastScrollY > 50);
      }
  ...
  function initPhotoGallery() {
    ...
    // NOTE: your HTML uses <p class="about-text"></p>.
    // We convert it to a <div class="about-text"> so we can safely inject <p> paragraphs.
    let aboutText = gallery.querySelector(".about-text");
    if (aboutText && aboutText.tagName === "P") {
      const div = document.createElement("div");
      div.className = aboutText.className;
      div.id = aboutText.id || "";
      aboutText.replaceWith(div);
      aboutText = div;
    }
    ...
    const stageCaption = gallery.querySelector("#stageCaption");
    ...
    function buildProjects(defs) {
      return defs.map((d) => {
        ...
        let images = [];
        let thumbs = [];
        const captions = Array.isArray(d.captions) ? [...d.captions] : [];
    ...
    function renderMobile() {
      ...
      projects.forEach((p, idx) => {
        ...
        function animateOpen(el) {
          const bodyEl = el.querySelector(".mobile-body");
  ...
  function initCodingSection() {
    ...
    if (!list || !preview || !mobile || !infoMsg) {
      return;
    }
    ...
    projects.forEach((p, i) => {
      ...
      <div class="coding-mobile-body">
        ...
        <p>${String(p.description).replace(/\n/g, "<br>")}</p>
      </div>
    `;
      ...
      head.addEventListener("click", () => {
        ...
        mobile.querySelectorAll(".coding-mobile-item").forEach((other) => {
          ...
          const b = other.querySelector(".coding-mobile-body");
  ...
  function initSectionTiming() {
    ...
    const io = new IntersectionObserver((entries) => {
      entries.forEach((e) => {
        ...
        else if (state.activeId === id && e.intersectionRatio <= 0.10) {
          leave(id);
        }
      });
    },
